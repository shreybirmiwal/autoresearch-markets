...
class LargeTradeFollowerStrategy(Strategy):
    """Follow large trades: if a large YES trade (big size at low price) happens,
    it may be informed buying - piggyback by buying YES too.
    Large trades at high prices may be informed NO sellers - buy NO."""
    name: str = "large_trade_follower"
    buy_yes_below: float = 0.36
    buy_no_above: float = 0.80
    size_percentile: float = 75.0  # require size above 75th percentile
    window: int = 50
    order_size: float = 1.0

    def __post_init__(self) -> None:
        self._size_hist: deque[float] = deque(maxlen=self.window)
    ...
    def on_event(self, state: dict[str, Any]) -> Order | None:
        ...
        if len(self._size_hist) < self.window:
            return None

        size_threshold = float(np.percentile(np.array(self._size_hist), self.size_percentile))
        is_large_trade = size >= size_threshold

        # Only trade on large trades (potential informed flow) at extreme prices
